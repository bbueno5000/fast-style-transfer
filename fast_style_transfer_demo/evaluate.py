...
def build_parser():
    ...
    parser.add_argument(
        ...
        type=str,
        dest='device',
        help='device to perform compute on',
        metavar='DEVICE',
        default=DEVICE)
    parser.add_argument(
        '--batch-size',
        type=int,
        dest='batch_size',
        help='batch size for feedforwarding',
        metavar='BATCH_SIZE',
        default=BATCH_SIZE)
    parser.add_argument(
...
def ffwd(data_in, paths_out, checkpoint_dir, device_t='/gpu:0', batch_size=4):
    ...
    curr_num = 0
    soft_config = tensorflow.ConfigProto(allow_soft_placement=True)
    soft_config.gpu_options.allow_growth = True
    with g.as_default(), g.device(device_t), \
            tensorflow.Session(config=soft_config) as sess:
        batch_shape = (batch_size,) + img_shape
        img_placeholder = tensorflow.placeholder(
            tensorflow.float32, shape=batch_shape, name='img_placeholder')
        preds = transform.net(img_placeholder)
        saver = tensorflow.train.Saver()
        if os.path.isdir(checkpoint_dir):
            ckpt = tensorflow.train.get_checkpoint_state(checkpoint_dir)
    ...
    if len(remaining_in) > 0:
        ffwd(
            ...
            checkpoint_dir, device_t=device_t, batch_size=1)
...
def ffwd_different_dimensions(
    ...
    DOCSTRING
    """
    in_path_of_shape = collections.defaultdict(list)
    out_path_of_shape = collections.defaultdict(list)
    ...
    for shape in in_path_of_shape:
        ...
        ffwd(
            in_path_of_shape[shape], out_path_of_shape[shape], 
...
def from_pipe(opts):
    ...
    pipe_in = subprocess.Popen(
        command, stdout=subprocess.PIPE, bufsize=10**9, stdin=None, stderr=None)
    command = [
        "ffmpeg",
        '-loglevel', "info",
    ...
    soft_config = tensorflow.ConfigProto(allow_soft_placement=True)
    soft_config.gpu_options.allow_growth = True
    with g.as_default(), g.device(opts.device), tensorflow.Session(config=soft_config) as sess:
        ...
        if os.path.isdir(opts.checkpoint):
            ckpt = tensorflow.train.get_checkpoint_state(opts.checkpoint)
            if ckpt and ckpt.model_checkpoint_path:
                saver.restore(sess, ckpt.model_checkpoint_path)
            else:
                raise Exception("No checkpoint found...")
    ...
    if not os.path.isdir(opts.in_path):
        ...
        if opts.allow_different_dimensions:
            ffwd_different_dimensions(
                full_in, full_out, opts.checkpoint_dir,
